...
bool Sodaq_nbIOT::connect(const char* apn, /* char* cdp, */const char* forceOperator)
{
    if (!on()) {
        return false;
    }
...
void Sodaq_nbIOT::cfun0()
{	
    println("AT+CFUN=0");    
}
void Sodaq_nbIOT::nconf1()
{	
    println("AT+NCONFIG=\"AUTOCONNECT\",\"FALSE\"");    
}
void Sodaq_nbIOT::nconf2()
{	
    println("AT+NCONFIG=\"CR_0354_0338_SCRAMBLING\",\"TRUE\"");    
}
void Sodaq_nbIOT::nconf3()
{	
    println("AT+NCONFIG=\"CR_0859_SI_AVOID\",\"TRUE\"");    
}
void Sodaq_nbIOT::cereg2()
{	
    println("AT+CEREG=2");    
}
void Sodaq_nbIOT::cgdcont1()
{	
    println("AT+CGDCONT=1, \"IP\",\"v-iot\"");    
}
void Sodaq_nbIOT::cfun1()
{	
    println("AT+CFUN=1");    
}
void Sodaq_nbIOT::cops1()
{	
    println("AT+COPS=1,2,\"29341\"");    
}
...
void Sodaq_nbIOT::ping()
{
    println("AT+NPING=\"8.8.8.8\"");
}


void Sodaq_nbIOT::cscon()
{	
	delay(10000);
    println("AT+CSCON=1");
...
void Sodaq_nbIOT::osocket()
{	
    println("AT+NSOCR=\"DGRAM\",17,13001,1");
	delay(3000);    
}

///NASTAVI IP IN PORT KAM POSLJE UDP PAKET///
void Sodaq_nbIOT::sendudp(String data, int size)
{	
////////////////////////////IP////////////port////
    print("AT+NSOST=0,\"000.000.000.000\",0000,");
	print(size);
	print(",\"");
	print(data); 
	println("\"");
