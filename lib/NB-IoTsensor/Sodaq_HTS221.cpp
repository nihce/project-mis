...
bool
Sodaq_HTS221::storeCalibration(void)
{
    uint8_t data;
    uint16_t tmp;
...
bool Sodaq_HTS221::deactivate(void)
{
    uint8_t data;
...
bool
Sodaq_HTS221::bduDeactivate(void)
{
    uint8_t data;
...
const double
Sodaq_HTS221::readHumidity(void)
{
    uint8_t data   = 0;
    uint16_t h_out = 0;
    double h_temp  = 0.0;
...
bool Sodaq_HTS221::writeRegister(byte slaveAddress, byte regToWrite, byte dataToWrite)
{
    Wire.beginTransmission(slaveAddress);
